...
class FriendsGet(Choreography):

    """
    Create a new instance of the FriendsGet Choreography. A TembooSession object, containing a valid
    set of Temboo credentials, must be supplied.
    """
    def __init__(self, temboo_session):
        Choreography.__init__(self, temboo_session, '/Library/Facebook/FriendsGet')


    def new_input_set(self):
        return FriendsGetInputSet()

    def _make_result_set(self, result, path):
        return FriendsGetResultSet(result, path)

    def _make_execution(self, session, exec_id, path):
        return FriendsGetChoreographyExecution(session, exec_id, path)

"""
An InputSet with methods appropriate for specifying the inputs to the FriendsGet
choreography. The InputSet object is used to specify input parameters when executing this choreo.
"""
class FriendsGetInputSet(InputSet):
        """
...
"""
A ResultSet with methods tailored to the values returned by the FriendsGet choreography.
The ResultSet object is used to retrieve the results of a choreography execution.
"""
class FriendsGetResultSet(ResultSet):
        """
...
class FriendsGetChoreographyExecution(ChoreographyExecution):
    
    def _make_result_set(self, response, path):
        return FriendsGetResultSet(response, path)
