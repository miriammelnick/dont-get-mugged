...
class StatusSet(Choreography):

    """
    Create a new instance of the StatusSet Choreography. A TembooSession object, containing a valid
    set of Temboo credentials, must be supplied.
    """
    def __init__(self, temboo_session):
        Choreography.__init__(self, temboo_session, '/Library/Facebook/StatusSet')


    def new_input_set(self):
        return StatusSetInputSet()

    def _make_result_set(self, result, path):
        return StatusSetResultSet(result, path)

    def _make_execution(self, session, exec_id, path):
        return StatusSetChoreographyExecution(session, exec_id, path)

"""
An InputSet with methods appropriate for specifying the inputs to the StatusSet
choreography. The InputSet object is used to specify input parameters when executing this choreo.
"""
class StatusSetInputSet(InputSet):
        """
...
"""
A ResultSet with methods tailored to the values returned by the StatusSet choreography.
The ResultSet object is used to retrieve the results of a choreography execution.
"""
class StatusSetResultSet(ResultSet):
        """
...
class StatusSetChoreographyExecution(ChoreographyExecution):
    
    def _make_result_set(self, response, path):
        return StatusSetResultSet(response, path)
