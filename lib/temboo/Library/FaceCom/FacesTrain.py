...
class FacesTrain(Choreography):

    """
    Create a new instance of the FacesTrain Choreography. A TembooSession object, containing a valid
    set of Temboo credentials, must be supplied.
    """
    def __init__(self, temboo_session):
        Choreography.__init__(self, temboo_session, '/Library/FaceCom/FacesTrain')


    def new_input_set(self):
        return FacesTrainInputSet()

    def _make_result_set(self, result, path):
        return FacesTrainResultSet(result, path)

    def _make_execution(self, session, exec_id, path):
        return FacesTrainChoreographyExecution(session, exec_id, path)

"""
An InputSet with methods appropriate for specifying the inputs to the FacesTrain
choreography. The InputSet object is used to specify input parameters when executing this choreo.
"""
class FacesTrainInputSet(InputSet):
        """
...
"""
A ResultSet with methods tailored to the values returned by the FacesTrain choreography.
The ResultSet object is used to retrieve the results of a choreography execution.
"""
class FacesTrainResultSet(ResultSet):
        """
...
class FacesTrainChoreographyExecution(ChoreographyExecution):
    
    def _make_result_set(self, response, path):
        return FacesTrainResultSet(response, path)
